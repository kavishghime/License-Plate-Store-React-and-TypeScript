...
export class CartView extends React.Component {
	...
	removeItemFromCart = (plate) => {
		removeFromCart(plate)
			.then(() => getCartContents())
			.then(data => this.setState({ cartContents: data }));
	}
			...
			rows = this.state.cartContents.map((licensePlate, index) => {
				return (
					<div key={licensePlate._id} className="col-md-4" style={{ backgroundColor: (index % 2 === 0) ? '#F5F5F5' : '' }}>
						<LicensePlate onButtonClicked={this.removeItemFromCart} currencyInfo={this.props.currencyInfo} plate={licensePlate} buttonText="Remove from cart &times;" />
					</div>
				);
			});
		}
